...
function checkTile(e) {
  const tile = e.target.closest(".home .tile, .project .tile");
  if (!tile) return;

  const title = tile.querySelector("[data-name], .projectName");
  ...
  isHomeTile ? checkWhichHomeTile(tile) : displayTask(tile.dataset.project);
  isHomeTile ? hideAddTaskBtn() : showAddTaskBtn();
}
